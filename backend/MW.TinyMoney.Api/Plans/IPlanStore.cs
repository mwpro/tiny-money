...
namespace MW.TinyMoney.Api.Plans
{
    ...
    public interface IPlanStore
    {
        ...
        Task<IEnumerable<PlanSummary>> GetActivePlans(DateTime today);
        ...
        Task<int> AddPlanTag(int planId, int tagId, decimal amount, string description);
        Task UpdatePlanTag(int planTagId, decimal amount, string description);
        Task DeletePlanTag(int planTagId);
    }
    ...
    public class MySqlPlanStore : IPlanStore
    {
        ...
        private const string GetActivePlansQuery =
            """
            SELECT p.id, p.title, p.description, p.date_from AS dateFrom, p.date_to AS dateTo,
                   COALESCE(SUM(pt.amount), 0) AS totalBudget,
                   COALESCE(SUM(
        ...
        private const string GetPlanDetailQuery =
            """
            SELECT p.id, p.title, p.description, p.date_from AS dateFrom, p.date_to AS dateTo,
                   pt.id AS planTagId, pt.id AS id, pt.tag_id AS tagId, tg.name AS tagName,
                   pt.amount, pt.description,
            ...
            WHERE p.id = @planId
            GROUP BY p.id, p.title, p.description, p.date_from, p.date_to,
                     pt.id, pt.tag_id, tg.name, pt.amount, pt.description;
            """;
        ...
        private const string DeletePlanTagQuery =
            """
            DELETE FROM plan_tag WHERE id = @planTagId;
            """;
        ...
        public async Task<IEnumerable<PlanSummary>> GetActivePlans(DateTime today)
        {
            await using var connection = _mySqlConnectionFactory.CreateConnection();
            await connection.OpenAsync();
            return await connection.QueryAsync<PlanSummary>(GetActivePlansQuery, new { today = today.Date });
        }
        ...
        public async Task<int> AddPlanTag(int planId, int tagId, decimal amount, string description)
        {
            await using var connection = _mySqlConnectionFactory.CreateConnection();
            await connection.OpenAsync();
            return await connection.QuerySingleAsync<int>(AddPlanTagQuery,
                new { planId, tagId, amount, description });
        }

        public async Task UpdatePlanTag(int planTagId, decimal amount, string description)
        {
            await using var connection = _mySqlConnectionFactory.CreateConnection();
            await connection.OpenAsync();
            await connection.ExecuteAsync(UpdatePlanTagQuery, new { planTagId, amount, description });
        }

        public async Task DeletePlanTag(int planTagId)
        {
            await using var connection = _mySqlConnectionFactory.CreateConnection();
            await connection.OpenAsync();
            await connection.ExecuteAsync(DeletePlanTagQuery, new { planTagId });
        }
    }
}
