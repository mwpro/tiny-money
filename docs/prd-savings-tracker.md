...
20. As a user, I want a chart of monthly cash flows per account (deposited and withdrawn each month) so that I can see my saving/spending patterns per account.
...
22. As a user, I want to navigate to any past month's snapshot and see or edit it so that I can correct mistakes or enter historical data.
23. As a user, I want archived accounts to be excluded from the monthly entry form but still visible in historical charts.
24. As a user, I want to manually archive a goal when I've achieved it (e.g. bought the car) so that it disappears from my active view without losing its history.
25. As a user, I want to unarchive a goal in case I archived it by mistake.
...
- **savings_goal_categories** — goal_id, category_id (many-to-many)
...
- **SavingsController** — one controller for the whole module, with routes for:
  ...
  - Reports endpoint: time-series totals, by-category totals, monthly cash flows, ROI per account, goal progress + projections
...
- **`features/savings/`** — new top-level feature; added to the main nav between Reports and Tags:
  ...
  - **Goals page** — CRUD for goals, link categories to goals, view goal progress cards with projections.
...
- **Goal completion**: no automatic detection. The user manually archives a goal (e.g. after buying the car). Archived goals are hidden from the active goals view and excluded from all calculations. They can be unarchived. Only active goals participate in progress/projection/cushion logic.
- **Goal progress**: for each active goal, sum the most recent balance for every active account in the goal's linked categories, then subtract the cushion target. The cushion is deducted from each goal independently (it's a floor/reservation on the pool, not a one-time split). Progress is floored at zero.
- **Goal projection**: (target_amount − current_progress) / average monthly net balance growth over the last 3–6 months across goal-linked accounts. Net balance growth = balance[n] − balance[n-1] per account per month.
...
1. **Goal progress calculation** — inputs: list of account snapshots + goal config + cushion settings → output: available amount. Key edge cases: cushion covers entire eligible balance (progress = 0), multiple goals sharing a category (each independently deducts full cushion), no snapshots yet (progress = 0).
2. **Goal projection** — inputs: balance history + goal config → output: projected date. Edge cases: negative average growth rate (no projection shown), exactly at target (show "achieved").
3. **ROI calculation** — (balance − cumulative invested) / cumulative invested. Covers: positive gain, negative gain, zero invested (ROI not shown), partial history with seed.
4. **Cushion recommendation** — average monthly expenses × multiplier. Tested with mock expense data.
...
- Goal categories can overlap: the same category can contribute to multiple goals. There is no splitting — if "Main savings" = 100k is linked to both "Car" (target 50k) and "Home" (target 500k), both goals show 100k available (minus the cushion, independently). This is intentional and transparent.
